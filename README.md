...
if cloudrunjob_mode:
    ...
    OUTPUT_TABLE_SUFFIX = '_218newstr'
    logger.info(f'OUTPUT_TABLE_SUFFIX: {OUTPUT_TABLE_SUFFIX}')
...
def process_minmax_data(restrict_minmax, dpt_list, tenpo_cd, bucket_name, sales_df, extract_as_df_with_encoding, time_log_list):
    ...
    if restrict_minmax:
        ...
        # If restrict_minmax is False, initialize the output DataFrames as empty or handle as needed
        # to ensure they are always returned.
        nominmax_prdcd_df = pd.DataFrame(columns=['PRD_CD', 'reason', 'HACCHU_TO_YMD'])
        minmax0_prdcd_df = pd.DataFrame(columns=['PRD_CD', 'reason', 'HACCHU_TO_YMD'])
...
def restrict_tenpo_hacchu_end_func(restrinct_tenpo_hacchu_end, tenpo_cd, bucket_name, sales_df, my_date, extract_as_df, time_log_list):
    ...
    if restrinct_tenpo_hacchu_end:
        ...
        # If restrinct_tenpo_hacchu_end is False, initialize the output DataFrame as empty
        # to ensure it is always returned.
        hacchu_teishi_prdcd_df2 = pd.DataFrame(columns=['PRD_CD', 'reason', 'HACCHU_TO_YMD'])
